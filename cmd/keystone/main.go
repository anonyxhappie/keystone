...
func usage(){fmt.Println("keystone init | status | ask <request> | doctor")}
func runInit(root string){
	name:=filepath.Base(root); caps:=project.Detect(root); p,err:=state.New(root).Init(name,caps); if err!=nil { fatal(err) }
	b,_:=json.MarshalIndent(p,"","  "); fmt.Println(string(b))
}
func runStatus(root string){
	var p map[string]any; if err:=state.New(root).Read("project.json",&p); err!=nil { fatal(fmt.Errorf("Keystone is not initialized: %w",err)) }; b,_:=json.MarshalIndent(p,"","  "); fmt.Println(string(b))
}
func runAsk(root string,args []string){
	if len(args)==0 { fatal(fmt.Errorf("ask requires a request")) }
	req:=strings.Join(args," "); o:=work.NewOrder(req); p:=context.Compile(root,work.Packet(o)); fmt.Println(harness.Render(p))
