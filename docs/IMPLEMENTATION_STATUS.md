...
This matrix is an audit of executable behavior on `main` for release `v2.1.3`. A capability is not marked implemented solely because a type or interface exists.
...
The PARTIAL entries are explicit local-first boundaries, not broken capabilities. Keystone fails closed when external or remote authority is required. All core capabilities, real provider bridges (Antigravity and Codex), and release verification criteria are satisfied for release `v2.1.3`.
